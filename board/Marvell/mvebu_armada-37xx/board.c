...
static int edpu_v2_last_stage_init(void)
{
	struct udevice *dev;
	int ret;
...
int last_stage_init(void)
{
	...
	if (of_machine_is_compatible("methode,edpu"))
		return edpu_v2_last_stage_init();
...
static int edpu_v2_fdt_setup(void *blob)
{
	const char *path;
	int ret;
